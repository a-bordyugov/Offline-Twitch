...
mod:hook_origin(TwitchGameMode, "_clear_used_votes", function(self, force_clear)
    ...
    if force_clear or num_available_vote_templates - table.size(used_vote_templates) <=
          TW_Tweaker._MIN_VOTES_LEFT_IN_ROTATION then
        table.clear(used_vote_templates)
    end
end)

mod:hook_safe(TwitchManager, "disconnect", function()
